...
class Product:
    ...
    def __new__(cls, *args, **kwargs):
        obj_link = super().__new__(cls)
        obj_link.id = cls.obj_cnt()
        return obj_link
    ...
    def __setattr__(self, key, value):
        ...
        elif key in ('weight', 'price') and (
                type(value) not in (int, float) or type(value) in (int, float) and value <= 0):
            raise TypeError("Неверный тип присваиваемых данных.")
...
# book = Product("Python ООП", 1, 1024)
