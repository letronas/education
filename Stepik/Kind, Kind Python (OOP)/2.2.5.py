...
class WindowDlg:
    def __init__(self, title, width, height):
        self.__title = title
        if self.check_digit(width):
            self.__width = width
        if self.check_digit(height):
    ...
    def height(self, height):
        height = self.check_digit(height)
        if height:
            self.__height = height
            self.show()
    ...
    def check_digit(digit):
        if type(digit) == int and 0 <= digit <= 10000:
            return digit


a = WindowDlg('Окно', 'a', 1)
