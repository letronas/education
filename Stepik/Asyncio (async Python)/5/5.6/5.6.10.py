...
async def execute_task(task_name, subtasks):
    subtask_name = subtasks['название']
    subtask_time = subtasks['время']
    task = execute_subtask(subtask_name, subtask_time)
    try:
        res = await asyncio.wait_for(task, timeout=5)
        return True, res
    except asyncio.TimeoutError:
        return False, f"Подзадача: {subtask_name} не успела выполниться в срок, за {subtask_time} сек."


async def main():
    for task_name in tasks_dependencies.keys():
        coros = [execute_task(task_name, stage) for stage in tasks_dependencies[task_name]['этапы']]
        # Возвращаем список из кортежей Флагов успешности и сообщения
        res = await asyncio.gather(*coros)
        # пытаемся понять были ли только успешные запуски или все же были и не успешные
        flg = set(flag for flag, msg in res)
         
        for flag, msg in res:
            print(msg)

        # если есть True и False, значит были падения
        if len(flg) == 2:
